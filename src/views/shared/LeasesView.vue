  ...
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 pb-4">
      ...
      <div class="flex items-center gap-3 flex-wrap">
        ...
        <input v-model="searchQuery" type="text" placeholder="Search tenant, unit..." class="text-xs border border-slate-200 rounded-lg px-3 py-1.5 w-44 bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-slate-400" />
      </div>
    </div>

    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
      <div class="stat-card"><div class="stat-icon bg-blue-50 text-blue-600"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg></div><div><p class="stat-label">Active</p><p class="stat-value text-blue-700">{{ activeLeases.length }}</p></div></div>
      <div class="stat-card"><div class="stat-icon bg-amber-50 text-amber-600"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></div><div><p class="stat-label">Under Notice</p><p class="stat-value text-amber-700">{{ underNoticeLeases.length }}</p></div></div>
        ...
        <table class="w-full text-left">
          <thead>
            <tr class="border-b border-slate-100 bg-slate-50/70">
              <th v-for="col in columns" :key="col.key" @click="col.sortable && sortBy(col.key)" :class="['px-4 py-3 text-[10px] font-extrabold uppercase tracking-wider text-slate-400 select-none whitespace-nowrap', col.sortable ? 'cursor-pointer hover:text-slate-600' : '']">
          ...
          <tbody>
            <tr v-for="l in pagedSortedLeases" :key="l.id" @click="openModal(l)" class="border-b border-slate-50 last:border-none hover:bg-blue-50/40 cursor-pointer transition-colors group">
              <td class="px-4 py-3"><p class="text-xs font-bold font-mono text-slate-800 group-hover:text-blue-700 transition-colors">{{ l.id }}</p></td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3"><span class="text-[10px] text-blue-500 font-semibold group-hover:text-blue-700 transition-colors">View →</span></td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <Transition name="modal-fade">
        <div v-if="selectedLease" class="modal-overlay" @click.self="closeModal">
          ...
          <div class="modal-panel">
            <div :class="['modal-accent', selectedLease.status === 'active' ? 'accent-blue' : selectedLease.status === 'notice_given' ? 'accent-amber' : 'accent-rose']"></div>
            <div class="p-6 space-y-6">
              ...
              <div class="grid grid-cols-3 gap-3">
                <div class="metric-card"><p class="metric-label">Monthly Rent</p><p class="metric-value text-blue-700">KES {{ (selectedLease.rent_amount || 0).toLocaleString() }}</p><p class="metric-sub">per month</p></div>
              ...
              <div class="grid grid-cols-2 gap-6">
                <div class="space-y-3">
                  ...
                  <div class="space-y-2">
                    ...
                    <div class="profile-row"><span class="profile-key">Tenant ID</span><span class="profile-val font-mono text-[10px]">{{ selectedLease.tenant_id }}</span></div>
                ...
                <div class="space-y-3">
                  ...
                  <div class="space-y-2">
                    <div class="profile-row"><span class="profile-key">Rent/mo</span><span class="profile-val text-blue-700">KES {{ (selectedLease.rent_amount || 0).toLocaleString() }}</span></div>
              ...
              <template v-if="selectedLease.status === 'active'">
                ...
                <div class="border-t border-slate-100 pt-4 space-y-3">
                  <button @click="showTransferForm = !showTransferForm" class="flex items-center gap-2 text-xs font-bold text-blue-600 hover:text-blue-800 transition-colors">
                  ...
                  <Transition name="slide-down">
                    <div v-if="showTransferForm" class="space-y-3 bg-blue-50 border border-blue-100 rounded-2xl p-4">
                      <p class="text-xs text-slate-500">Select a destination unit. The current lease will be terminated and a new one generated.</p>
                      <div><label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Available Unit</label>
                        <select v-model="targetUnitId" class="w-full border border-slate-200 rounded-lg p-2 text-xs focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white"><option value="" disabled>-- Select Unit --</option><option v-for="u in availableUnits" :key="u.id" :value="u.id">{{ u.name }} (KES {{ u.rent_amount.toLocaleString() }}/mo)</option></select>
                      </div>
                      <div class="flex justify-end gap-2"><button @click="showTransferForm = false" class="px-4 py-2 border border-slate-200 text-slate-600 font-bold rounded-xl text-xs hover:bg-white transition-colors">Cancel</button><button @click="submitTransfer" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-xs shadow-sm">Process Transfer</button></div>
                    </div>
                  </Transition>
                </div>
              </template>
    ...
    <!-- Compiled Lease Agreement Modal -->
    <Teleport to="body">
      <Transition name="modal-fade">
        <div v-if="showAgreementModal" class="modal-overlay" @click.self="showAgreementModal = false">
          ...
          <div class="modal-panel max-w-lg">
            ...
            <div class="p-6 space-y-6">
              ...
              <!-- Contract View Area -->
              <div class="bg-slate-50 border border-slate-100 rounded-xl p-4 font-mono text-[10px] text-slate-700 leading-relaxed max-h-96 overflow-y-auto whitespace-pre-wrap">
                {{ compiledAgreementText }}
              </div>

              <div class="flex justify-end gap-3 pt-3 border-t border-slate-100">
                <button type="button" @click="showAgreementModal = false" class="px-4 py-2 border border-slate-200 text-slate-600 font-bold rounded-xl text-xs">Close</button>
                <button type="button" @click="window.print()" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-xs shadow-sm flex items-center gap-1.5">
                  🖨️ Print Agreement
                </button>
              </div>
            </div>
...
export default {
  ...
  setup() {
    ...
    const searchQuery = ref(''); const statusFilter = ref(''); const sortKey = ref('start_date'); const sortDir = ref('desc');
    const selectedLease = ref(null); const showVacateForm = ref(false); const showTransferForm = ref(false);
    const vacateDate = ref(''); const vacateReason = ref(''); const targetUnitId = ref('');
    const showAgreementModal = ref(false);
    const compiledAgreementText = ref('');
    const columns = [
      { key: 'id', label: 'Lease ID', sortable: true }, { key: 'tenant_name', label: 'Tenant', sortable: true },
      { key: 'unit_name', label: 'Unit', sortable: false }, { key: 'rent_amount', label: 'Rent/mo', sortable: true },
      { key: 'start_date', label: 'Start', sortable: true }, { key: 'end_date', label: 'End', sortable: true },
      { key: '_days', label: 'Days Left', sortable: false }, { key: 'status', label: 'Status', sortable: true }, { key: '_a', label: '', sortable: false }
    ];
    ...
    const filteredLeases = computed(() => {
      ...
      if (searchQuery.value.trim()) { const q = searchQuery.value.toLowerCase(); list = list.filter(l => (l.id||'').toLowerCase().includes(q)||(l.tenant_name||l.tenant_id||'').toLowerCase().includes(q)||(l.unit_name||l.unit_id||'').toLowerCase().includes(q)); }
      return list;
    });
    const sortedLeases = computed(() => [...filteredLeases.value].sort((a,b) => { const av=a[sortKey.value],bv=b[sortKey.value]; if(av===bv)return 0; return (sortDir.value==='asc'?1:-1)*(av>bv?1:-1); }));
    const sortBy = k => { if(sortKey.value===k)sortDir.value=sortDir.value==='asc'?'desc':'asc'; else{sortKey.value=k;sortDir.value='asc';} };
    ...
    const statusBadge = s => { const b='text-[9px] uppercase tracking-wider font-extrabold px-2.5 py-1 rounded-full border '; return b+({active:'bg-emerald-100 text-emerald-700 border-emerald-200',notice_given:'bg-amber-100 text-amber-700 border-amber-200',terminated:'bg-rose-100 text-rose-700 border-rose-200'}[s]||'bg-slate-100 text-slate-600 border-slate-200'); };
    const avatarColor = n => { const c=['linear-gradient(135deg,#6366f1,#8b5cf6)','linear-gradient(135deg,#0ea5e9,#2563eb)','linear-gradient(135deg,#10b981,#059669)','linear-gradient(135deg,#f59e0b,#d97706)','linear-gradient(135deg,#ec4899,#be185d)','linear-gradient(135deg,#14b8a6,#0d9488)']; let h=0; for(let i=0;i<(n||'').length;i++)h=(n||'').charCodeAt(i)+((h<<5)-h); return c[Math.abs(h)%c.length]; };
    ...
    const submitVacate = async () => { if(!vacateDate.value||!vacateReason.value){store.error='Please fill out all fields';return;} try{await store.vacateLease(selectedLease.value.id,vacateReason.value,vacateDate.value);selectedLease.value.status='notice_given';showVacateForm.value=false;store.success='Notice to vacate submitted.';}catch(e){store.error=e.message;} };
    const submitTransfer = async () => { if(!targetUnitId.value){store.error='Please select a unit';return;} try{const nl=await store.transferTenant(selectedLease.value.id,targetUnitId.value);selectedLease.value.status='terminated';store.leases.push(nl);showTransferForm.value=false;store.success='Tenant transferred.';}catch(e){store.error=e.message;} };
...
This Agreement is made on this ${start} by and between:

LANDLORD: REOS Properties Management (representing Owner ID: ${lease.landlord_id})
TENANT: ${lease.tenant_name || lease.tenant_id}

1. PROPERTY PREMISES
The Landlord agrees to lease, and the Tenant agrees to rent the premises located at:
Unit: ${lease.unit_name || lease.unit_id}
Property: ${lease.property_name || 'REOS Assigned Property Estate'}

2. TERM OF LEASE
The tenancy shall commence on ${start} and shall continue until ${end} unless terminated earlier in accordance with the terms of this Agreement.

3. RENT & DEPOSITS
* Monthly Rent: KES ${rent} payable on or before the 5th day of every calendar month.
* Security Deposit: KES ${dep} to be held by the Landlord and refunded at lease termination, subject to move-out inspections.

4. COVENANTS
* The Tenant shall keep the premises in a clean and sanitary condition.
* The Tenant shall not sub-let the premises without written consent from the Landlord.
* The Landlord reserves the right to inspect the premises upon giving 24 hours notice.
...
Tenant Signatory: _________________________`;
    ...
    onMounted(() => {
      if(store.leases.length===0) store.leases=[
    ...
    const { paginatedItems: pagedSortedLeases, currentPage, totalPages, totalItems, startItem, endItem, pageNumbers, pageSize, prevPage, nextPage, goToPage } = usePagination(sortedLeases);
        return { store,searchQuery,statusFilter,sortKey,sortDir,columns,leases,activeLeases,underNoticeLeases,terminatedLeases,totalMonthlyRent,filteredLeases,sortedLeases,selectedLease,showVacateForm,showTransferForm,vacateDate,vacateReason,targetUnitId,availableUnits,sortBy,formatDate,daysLeft,leaseDuration,totalLeaseValue,statusLabel,statusBadge,avatarColor,openModal,closeModal,submitVacate,submitTransfer,showAgreementModal,compiledAgreementText,openAgreementCompiler, pagedSortedLeases, currentPage, totalPages, totalItems, startItem, endItem, pageNumbers, pageSize, prevPage, nextPage, goToPage };
  }
};
</script>
...
.accent-blue{background:linear-gradient(90deg,#60a5fa,#2563eb)}
